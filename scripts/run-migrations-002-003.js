...
async function runMigration(migrationNumber, description) {
  ...
  try {
    ...
    if (error) {
      ...
      console.error(
        '⚠️  exec_sql function not available. Please run the migration manually in Supabase Dashboard.'
      )
...
async function main() {
  ...
  if (!correctionsExists || !userProgressExists) {
    ...
    if (!error && policies) {
      ...
      const hasOldPolicy = policies.some(
        (p) =>
  ...
  console.log('\n\n📌 Summary:')
  console.log(
    'After running the required migrations, the analyze-errors functionality should work properly.'
  )
